...
问题：{question}
选项：{choices}
请从选项中找到该问题的答案并输出A，B，C，D中的一个字母表示你认为正确的选项，注意，不要输出任何多余的文字。
答案：
{answer}
"""
...
class Dataset(ParentDataset):
    ...
    def analyse_file(self, fp: TextIO, sample_num: Optional[int] = None) -> List[Tuple[Any, Any]]:
        ...
        print(f"{BOLD_GREEN}ncr-xdw 数据集的文章统一截断为{BOLD_RED}{TRUNCATED_LENGTH}{BOLD_GREEN}，以避免长度超限{RESET_COLOR}")
        original_data = json.load(fp)
        data = []
        n = len(original_data)
        if sample_num is not None:
            assert sample_num > 0, f"`sample_num` must be positive integer, but get {sample_num}"
        else:
            sample_num = n

        _data_samples = original_data[:sample_num]
        for s in _data_samples:
            content = s['Content'][:TRUNCATED_LENGTH]
            questions = s['Questions']
            for question in questions:
                q = question['Question']
                # idx = re.search(r'[\u4e00-\u9fa5]', q).start()
                # q = q[idx:]
